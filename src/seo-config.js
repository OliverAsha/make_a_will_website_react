const SITE_NAME = 'Make a Will';
const SITE_URL = 'https://www.makeawill.co.uk';

// Central mapping of every route to its SEO metadata.
// Used by both the React SEO component (client-side) and
// the post-build script (to pre-bake meta tags into static HTML).
const seoConfig = {
  // Main pages
  '/': {
  ...
  // Resource pages — Executors & Probate
  '/resource/executors': {
  ...
  // Resource pages — Lasting Power of Attorney
  ...
  // Resource pages — Family Situations
  ...
  '/dying-without-a-will-intestacy': {
    ...
    canonical: '/resource/dying-without-a-will-intestacy'
  },
...
// Helper: get SEO data for a path, with fallback
function getSEO(path) {
  if (seoConfig[path]) {
    const config = { ...seoConfig[path] };
    if (config.canonical) {
      config.canonicalUrl = `${SITE_URL}${config.canonical}`;
    }
    return config;
  }

  // Try matching resource pages by slug
  const resourceMatch = path.match(/^\/resources?\/(.+)/);
  if (resourceMatch) {
    const withResource = `/resource/${resourceMatch[1]}`;
    if (seoConfig[withResource]) return seoConfig[withResource];
  }

  // Try matching topic pages
  if (path.startsWith('/topic/')) {
    const category = path.replace('/topic/', '').replace(/-/g, ' ');
    return {
      title: `${category.charAt(0).toUpperCase() + category.slice(1)} | Make a Will Blog`,
      description: `Browse our articles about ${category}. Advice and guidance from our team of legal experts.`
    };
  }

  // Default fallback
  return {
    title: 'Make a Will | Solicitor-Checked Wills',
    description: 'Create a solicitor-checked will online in as little as 15 minutes. Trusted since 2008, with lifetime updates and a money-back guarantee.'
  };
}

// All routes that should have static HTML pages generated
const allRoutes = Object.keys(seoConfig);

export { seoConfig, getSEO, allRoutes, SITE_NAME, SITE_URL };
